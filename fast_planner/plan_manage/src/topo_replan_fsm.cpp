...
void TopoReplanFSM::checkCollisionCallback(const ros::TimerEvent& e)
{
    ...
    if (false)
    {
        ...
        double dist = planner_manager_->pp_.dynamic_ ? edt_env->evaluateCoarseEDT(target_point_, /* time to program start */ info->duration_)
            ...
            for (double r = dr; r <= 5 * dr + 1e-3; r += dr)
            {
                for (double theta = -90; theta <= 270; theta += dtheta)
                {
                    for (double nz = 1 * dz; nz >= -1 * dz; nz -= dz)
                    {
                        ...
                        dist = planner_manager_->pp_.dynamic_ ? edt_env->evaluateCoarseEDT(new_pt, /* time to program start */ info->duration_)
...
bool TopoReplanFSM::callTopologicalTraj(int step)
{
    ...
    if (plan_success)
    {
        ...
        visualization_->drawBspline(locdat->position_traj_, 0.08, Eigen::Vector4d(1.0, 0.0, 0.0, 1), false, 0.15, Eigen::Vector4d(1.0, 1.0, 1.0, 1), 99, 99);
