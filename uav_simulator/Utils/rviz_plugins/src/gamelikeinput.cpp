    ...
    : rviz::SelectionTool(), move_tool_(new rviz::InteractionTool()), selecting_(false), sel_start_x_(0), sel_start_y_(0), moving_(false)
{
    ...
    topic_property_wp_ = new rviz::StringProperty("TopicPoint", "point_list", "The topic on which to publish navigation goals.", getPropertyContainer(),
                                                  SLOT(updateTopic()), this);

    topic_property_drone_ = new rviz::StringProperty("TopicSelect", "select_list", "The topic on which to publish select drone id.", getPropertyContainer(),
                                                     SLOT(updateTopic()), this);

    topic_property_swarm_ =
        new rviz::StringProperty("TopicSwarm", "swarm", "The topic on which to publish swarm command.", getPropertyContainer(), SLOT(updateTopic()), this);
...
void GameLikeInput::onPoseSet(double x, double y, double z, double theta)
{
    ...
    ROS_INFO("Setting goal: Frame:%s, Position(%.3f, %.3f, %.3f), "
             ...
             fixed_frame.c_str(), goal.pose.position.x, goal.pose.position.y, goal.pose.position.z, goal.pose.orientation.x, goal.pose.orientation.y,
    ...
    if (selecting_)
    {
        ...
        if (event.leftUp())
        {
            ...
            for (auto it = selection.begin(); it != selection.end();)
            {
                rviz::Picked            p                                                                      = it->second;
                rviz::SelectionHandler* sh                                                                     = sel_manager->getHandler(p.handle);
                void (rviz::SelectionHandler::*vptr)(const rviz::Picked& obj, rviz::Property* parent_property) = &rviz::SelectionHandler::createProperties;
